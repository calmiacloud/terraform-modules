...
if [[ -z "$IMAGEBUILDER_VERSION_ARN" || "$IMAGEBUILDER_VERSION_ARN" == "None" ]]; then
  echo -e "\e[33m ==> No existing Image Builder version found.\e[0m"
else
  echo -e "\e[33m ==> Found Image Builder Version ARN: $IMAGEBUILDER_VERSION_ARN\e[0m"

  # ——— Paso 1: obtener el AMI_ID ———
  AMI_ID=$(
    aws imagebuilder list-image-pipeline-images \
      --image-pipeline-arn "$PIPELINE_ARN" \
      --no-paginate \
  ...
  # ——— Paso 2: describir y borrar snapshot ———
